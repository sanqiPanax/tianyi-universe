...
const YUAN = {
  ...
  // 原叫「二十八宿」—— 但二十八宿现在是真实的分野坐标(见 data/fenye.js),
  // 再拿它当群像的组名会打架,故改为「列国」。
  qun:"列国·群雄", jizhi:"客星", zahu:"荧惑", cast:"杂星",
};
...
let link, node, glow, activeId = null;
let viewMode = "force";          // force = 关系图 | fenye = 分野图(二十八宿重排)
...
function rebuild(nodes, links, gentle) {
  ...
  /* ---- 辉光层(在连线之下,让关系网能看清) ---- */
  glow = gGlow.selectAll("circle.glow").data(vnodes, d => d.id);
  glow.exit().remove();
  ...
  if (viewMode === "fenye") applyFenyeInstant();
  simulation.on("end", () => {
    scheduleCull(80);
...
/* ============================================================
   七点五、分野视图 —— 把星图重排进二十八宿
   ------------------------------------------------------------
   州依《汉书·地理志》十二次分野(有据可依那一层);
   州内进哪个宿 = 把该州的梗按剧集时序摊到它的 2~3 个宿上(无独立文献依据,已在图例注明)。
   中宫(天意机制)不进二十八宿 —— 二十八宿本就在紫微垣之外。
   ============================================================ */
const FENYE = window.DATA_FENYE || null;
const XIU_ORDER = "角亢氐房心尾箕斗牛女虚危室壁奎娄胃昴毕觜参井鬼柳星张翼轸".split("");
const XIU_ZHOU = {};
const NODE_XIU = {};
if (FENYE) {
  FENYE.ci.forEach(c => c.xiu.forEach(x => { XIU_ZHOU[x] = c.zhou; }));
  for (const x in FENYE.xiu) FENYE.xiu[x].forEach(id => { NODE_XIU[id] = x; });
  (FENYE.center || []).forEach(id => { NODE_XIU[id] = "中宫"; });
}

const GEO = { r0: 250, r1: 560, zhouLabelR: 196, coreRingR: 168 };
function sectorAngle(i) { return (-90 + i * (360 / 28)) * Math.PI / 180; }

/** 算出每颗星在分野视图里该待的位置 */
function fenyeTargets() {
  const cx = innerWidth / 2, cy = innerHeight / 2;
  const byXiu = {};
  XIU_ORDER.forEach(x => { byXiu[x] = []; });
  const centerNodes = [];
  vnodes.forEach(d => {
    const x = NODE_XIU[d.id];
    if (!x || x === "中宫") { centerNodes.push(d); return; }
    if (byXiu[x]) byXiu[x].push(d);
  });
  const t = {};
  XIU_ORDER.forEach((x, i) => {
    const arr = byXiu[x], n = arr.length, a = sectorAngle(i);
    arr.forEach((d, j) => {
      const rr = n <= 1 ? (GEO.r0 + GEO.r1) / 2
        : GEO.r0 + j * (GEO.r1 - GEO.r0) / (n - 1);
      t[d.id] = { x: cx + Math.cos(a) * rr, y: cy + Math.sin(a) * rr, a, r: rr };
    });
  });
  const m = Math.max(6, centerNodes.length);
  centerNodes.forEach((d, j) => {
    const a = -Math.PI / 2 + j * (2 * Math.PI / m);
    t[d.id] = { x: cx + Math.cos(a) * GEO.coreRingR, y: cy + Math.sin(a) * GEO.coreRingR, a, center: true };
  });
  return t;
}
...
function redrawAll() {
  if (!node) return;
  node.attr("transform", d => `translate(${d.x},${d.y})`);
  if (glow) glow.attr("cx", d => d.x).attr("cy", d => d.y);
  if (link) link.attr("d", d => edgePath(d));
}

let layoutTimer = null;
function animateLayout(targets, dur) {
  if (layoutTimer) { layoutTimer.stop(); layoutTimer = null; }
  const start = {};
  vnodes.forEach(d => { start[d.id] = { x: d.x, y: d.y }; });
  simulation.stop();
  const t0 = performance.now();
  layoutTimer = d3.timer(() => {
    const raw = Math.min(1, (performance.now() - t0) / dur);
    const e = d3.easeCubicInOut(raw);
    for (const d of vnodes) {
      const s = start[d.id], g = targets[d.id];
      if (!s || !g) continue;
      d.x = s.x + (g.x - s.x) * e;
      d.y = s.y + (g.y - s.y) * e;
    }
    redrawAll();
    if (raw >= 1) { layoutTimer.stop(); layoutTimer = null; }
  });
}

/* 分野视图的辅助线:28 条分界 + 两圈范围 + 十二州名 */
const gFenye = gDecor.append("g").attr("class", "fenye-guides");
function drawFenyeGuides() {
  gFenye.selectAll("*").remove();
  for (let i = 0; i < 28; i++) {
    const a = sectorAngle(i - 0.5);
    gFenye.append("line")
      .attr("x1", Math.cos(a) * (GEO.r0 - 62)).attr("y1", Math.sin(a) * (GEO.r0 - 62))
      .attr("x2", Math.cos(a) * (GEO.r1 + 34)).attr("y2", Math.sin(a) * (GEO.r1 + 34))
      .attr("stroke", "#c9a227").attr("stroke-opacity", .13).attr("stroke-width", 1);
  }
  [GEO.r0 - 62, GEO.r1 + 34].forEach(r => {
    gFenye.append("circle").attr("r", r).attr("fill", "none").attr("stroke", "#c9a227")
      .attr("stroke-opacity", .12).attr("stroke-width", 1).attr("stroke-dasharray", "2 9");
  });
  let idx = 0;
  FENYE.ci.forEach(c => {
    const mid = idx + (c.xiu.length - 1) / 2;
    idx += c.xiu.length;
    const a = sectorAngle(mid);
    const cnt = c.xiu.reduce((s, x) => s + ((FENYE.xiu[x] || []).length), 0);
    gFenye.append("text")
      .attr("x", Math.cos(a) * GEO.zhouLabelR).attr("y", Math.sin(a) * GEO.zhouLabelR)
      .attr("text-anchor", "middle").attr("dominant-baseline", "middle")
      .attr("font-family", '"Songti SC","STSong",serif').attr("font-size", 16)
      .attr("fill", "#c9a227").attr("fill-opacity", cnt ? .7 : .28)
      .text(cnt ? `${c.zhou} ${cnt}` : `${c.zhou} ·无梗`);
  });
}
drawFenyeGuides();

/** 分野视图里标签改沿径向摆到节点两侧(否则会压在方位线上) */
function styleLabels(t) {
  if (!node) return;
  if (viewMode === "fenye" && t) {
    node.select("text.lbl")
      .attr("text-anchor", d => { const g = t[d.id]; return (!g || g.center) ? "middle" : (Math.cos(g.a) >= 0 ? "start" : "end"); })
      .attr("dx", d => { const g = t[d.id]; return (!g || g.center) ? 0 : (Math.cos(g.a) >= 0 ? (d.r + 12) : -(d.r + 12)); })
      .attr("dy", d => (t[d.id] && !t[d.id].center) ? 5 : d.r + 13);
  } else {
    node.select("text.lbl").attr("text-anchor", "middle").attr("dx", 0).attr("dy", d => d.r + 13);
  }
}

/** 筛选/换数据重建后,若还在分野视图,直接把新节点摆好(不动画) */
function applyFenyeInstant() {
  const t = fenyeTargets();
  vnodes.forEach(d => { const g = t[d.id]; if (g) { d.fx = g.x; d.fy = g.y; d.x = g.x; d.y = g.y; } });
  simulation.stop();
  styleLabels(t);
  redrawAll();
}
...
function setView(m) {
  if (!FENYE || m === viewMode) return;
  viewMode = m;
  document.body.classList.toggle("fenye", m === "fenye");
  const btn = document.getElementById("btn-fenye");
  if (btn) btn.classList.toggle("on", m === "fenye");
  if (m === "fenye") {
    const t = fenyeTargets();
    vnodes.forEach(d => { const g = t[d.id]; if (g) { d.fx = g.x; d.fy = g.y; } });
    styleLabels(t);
    animateLayout(t, 1000);
    setTimeout(() => { if (viewMode === "fenye") fitView(900); }, 1040);
  } else {
    vnodes.forEach(d => { if (!d.core) { d.fx = null; d.fy = null; } });
    styleLabels(null);
    simulation.alpha(.75).restart();
    setTimeout(() => { if (viewMode === "force") fitView(900); }, 600);
  }
  scheduleCull(1300);
}
...
function nodeHTML(d) {
  ...
  const xiu = NODE_XIU[d.id];
  if (xiu && xiu !== "中宫") {
    html += `<div class="field"><b>分野</b> · ${esc(XIU_ZHOU[xiu] || "")} · ${esc(xiu)}宿<span class="muted" style="font-size:11.5px">(州依《汉书·地理志》十二次分野)</span></div>`;
  } else if (xiu === "中宫") {
    html += `<div class="field"><b>分野</b> · 中宫<span class="muted" style="font-size:11.5px">(天意机制不入二十八宿)</span></div>`;
  }
...
document.getElementById("btn-fenye").onclick = () => setView(viewMode === "fenye" ? "force" : "fenye");
...
document.getElementById("btn-play").onclick = () => {
  ...
  bgmPlay();                         // 漫游开始 → BGM 起(仍借这次点击当用户手势)
};
...
// BGM 只在「漫游」进行时响(用户 2026-09-13 要求):空闲看图时保持安静,
// 播片模式因为要放原片台词,同样不叠 BGM。
function roaming() { return roamTimer !== null; }
function bgmWanted() { return bgmOn && roaming() && !bgmHold && !filmMode && !document.hidden; }
...
function bgmPaint() {
  ...
  b.title = bgmOn
    ? "关羽之歌 · 漫游时自动播放(点击关闭背景音乐)"
    : "背景音乐已关闭 · 漫游时也不会响(点击开启)";
}
...
addEventListener("keydown", (e) => {
  ...
  else if (k === "g") { setView(viewMode === "fenye" ? "force" : "fenye"); }
